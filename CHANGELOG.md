...
### Added — Task 1: Reference Target Suite (v0.18.2 hardening pass)

- **25 new tests** in `tests/test_reference_targets.py` (total: 44 tests, +25 over previous 19):
  - Parametrized `/health` HTTP endpoint test for all 12 targets via `TargetRunner` (12 tests).
  - Parametrized canonical refusal-string assertion for all 6 hardened targets (6 tests).
  - Parametrized benign pass-through assertion for all 6 vulnerable targets (6 tests).
  - `TargetRunner` KeyError guard for unknown target names (1 test).
- **Verified with real terminal output**: `sql_vulnerable /health`, `sql_vulnerable /ground_truth`, `sql_vulnerable /chat` (injection echoed), `sql_hardened /chat` (refusal), `crucible target list` (all 12), `crucible target validate` (ALL TARGETS HEALTHY).

### Fixed

- **eBPF CLI platform disclosure**: `crucible ebpf --help` now shows platform warning at the top of the output.
- **Semantic Noise Sensitivity Mapping rename**: Removed all instances of `Embedding Boundary Mapping` from `crucible/`, `docs/`, `README.md`.
- **PyRIT competitive claims**: Added accurate strengths/limitations checklist in `docs/comparison.md`.
- **MCP transport matrix**: Created `docs/mcp-transport-coverage.md`; added `Known Limitations` section to `README.md`.
- **CGAF docstring**: Clarified that the adaptive fuzzer uses semantic refusal detection, not token log-probabilities.

## [0.18.1] - 2026-07-19

### Fixed

- **STIX 2.1 Spec Compliance**: Fixed STIX Indicator objects generated by `STIXReporter` to comply with the OASIS specification:
  - Updated prompt-pattern syntax to use custom `[x-ai-prompt:payload_sha256 = '{sha256}']` instead of `file` hash syntax.
  - Set `indicator_types` strictly to `["malicious-activity"]`.
  - Added the standard `confidence` field mapped from finding severity levels (`critical` -> 95, `high` -> 80, `medium` -> 50, `low` -> 20, `info` -> 10).
  - Added support for `confidence` inside the `STIXIndicator` Pydantic model.
- **GitHub Action Marketplace**: Shortened the action description in `action.yml` to under 125 characters to satisfy Marketplace publishing limits.

## [0.18.0] - 2026-07-14

### Added — Task G7: Ground-Truth Detection Accuracy Metrics

- **`crucible/benchmark/` package** — ground-truth detection accuracy benchmarking:
  - `accuracy.py` — `AccuracyBenchmark` class orchestrating programmatic scans against all 12 reference targets (6 vulnerable, 6 hardened) using category-to-module filtering. Calculates Precision, Recall, F1, and Accuracy, along with 95% bootstrap confidence intervals based on joint distribution resampling. Exports structured JSON report and Markdown report (`docs/accuracy_report.md`).
- **`crucible benchmark` CLI command sub-app** (`crucible/cli.py`):
  - `crucible benchmark accuracy [--repetitions INT] [--model STR] [--format-preset STR] [--output FILE] [--concurrency INT]` — runs the accuracy benchmark and displays a beautiful styled table of metrics.
- **`tests/test_accuracy.py`** — test suite validating bootstrap CI calculations and CLI/report generation.
