...
def _count_failed_by_severity(
    ...
    return sum(
        1
        for f in findings
        if not f.passed and f.severity == severity
    )
...
def finalize_scan_result(result: ScanResult) -> ScanResult:
    ...
    all_findings = [
        f for m in result.modules for f in m.findings
    ]
    failed = [f for f in all_findings if not f.passed]

    result.total_findings = len(failed)
    result.critical_count = _count_failed_by_severity(
        all_findings, Severity.CRITICAL
    )
    result.high_count = _count_failed_by_severity(
        all_findings, Severity.HIGH
    )
    result.medium_count = _count_failed_by_severity(
        all_findings, Severity.MEDIUM
    )
    result.low_count = _count_failed_by_severity(
        all_findings, Severity.LOW
    )
    result.info_count = _count_failed_by_severity(
        all_findings, Severity.INFO
    )
    ...
    result.duration_seconds = round(
        sum(m.duration_seconds for m in result.modules), 3
    )
