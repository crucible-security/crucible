...
def test_custom_cors_origins() -> None:
    os.environ["CRUCIBLE_ALLOWED_ORIGINS"] = "https://dashboard.crucible.enterprise,https://admin.crucible.enterprise"
    try:
        importlib.reload(crucible.core.api)
        ...
        response = client.get("/", headers=headers)
        assert response.status_code == 200
        assert response.headers.get("access-control-allow-origin") == "https://dashboard.crucible.enterprise"
